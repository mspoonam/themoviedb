//
//  GroupExtensionsSampleApp.swift
    ...
    public var date: Date? {
        get {
            ...
            case .string:
                ...
                return DtHandler.SharedFormatter.date(from: dateString)
            default:
                return nil
            }
        }
        set {
            if let newValue = newValue {
                self.object = DtHandler.SharedFormatter.string(from: newValue)
            } else {
                self.object = NSNull()
            }
        }
...
extension UIImageView {
    ...
    func setImage(fromDBPath path: ImgPath, withSize size: ImgSize, animatedOnce: Bool = true, withPlaceholder placeholder: UIImage? = nil) {
        guard let imageURL = ApiHelper.instance.imgHandler?.url(fromDBPath: path, withSize: size)
            else {
                return
        }
